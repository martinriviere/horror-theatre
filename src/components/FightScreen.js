...
class FightScreen extends Component {
  ...
  playerAttack = () => {
    let att = Math.ceil(
      Math.random() * (this.state.playerAtt - this.state.playerAttMin + 1) +
        this.state.playerAttMin
    );
    this.setState({
      action: "Tu attaques!",
      valeur: att,
    ...
    if (this.state.monsterDef - att > 0) {
      this.setState({
        ...
        resultat: `${this.state.monsterName} tombe a ${this.state.monsterDef -
          att}pv`
      });
    } else {
      this.setState({
  ...
  playerPowerUp = () => {
    ...
    this.setState({
      ...
      resultat: `Ta puissance d'attaque est de ${this.state.playerAttMin +
        pUp} / ${this.state.playerAtt + pUp}`
    });
  };
    ...
    return (
      <div>
        ...
        <div className="container" style={{ backgroundImage: `url(${BG})` }}>
          {endGame && (
            <div className="endGame" style={{ backgroundImage: `url(${BG})` }}>
              <p>{winner}</p>
              <button onClick={this.context.closeFight}>
